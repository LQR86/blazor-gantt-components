...
/// <summary>
/// MonthWeek50px pattern implementation for TimelineView.
...
{
    /// <summary>
    /// Main rendering method for MonthWeek50px pattern.
    /// Combines primary (month-year) and secondary (week start dates) headers.
    /// </summary>
    /// <returns>Complete SVG markup for MonthWeek50px headers</returns>
    private string RenderMonthWeek50pxHeaders()
    {
        try
        {
            Logger.LogDebugInfo($"MonthWeek50px pattern rendering - StartDate: {StartDate}, EndDate: {EndDate}, DayWidth: {DayWidth}");

            var primaryHeader = RenderMonthWeek50pxPrimaryHeader();
            var secondaryHeader = RenderMonthWeek50pxSecondaryHeader();

            return $@"
                <!-- MonthWeek50px Pattern Headers -->
                <g class=""monthweek-50px-headers"">
                    {primaryHeader}
                    {secondaryHeader}
                </g>";
        }
        ...
        {
            Logger.LogError($"Error rendering MonthWeek50px headers: {ex.Message}");
            return $"<!-- Error in MonthWeek50px pattern: {ex.Message} -->";
        }
    }
    ...
    private string RenderMonthWeek50pxPrimaryHeader()
    {
    ...
    private string RenderMonthWeek50pxSecondaryHeader()
    {
    ...
    /// <summary>
    /// Generates week periods for MonthWeek50px pattern.
        ...
        while (current <= EndDate)
        {
            ...
            if (visibleStart <= visibleEnd)
            {
                var period = new HeaderPeriod
                {
                    ...
                    Label = FormatMonthWeek50pxWeekStartDate(weekStart)
                };
    ...
    /// <summary>
    /// Formats a month-year for display in the MonthWeek50px primary header.
    ...
    private string FormatMonthWeek50pxMonthYear(DateTime date)
    {
        ...
        {
            Logger.LogError($"Error formatting MonthWeek50px month-year {date:yyyy-MM-dd}: {ex.Message}");
            return $"{date:MMM yyyy}";
        }
    }

    /// <summary>
    /// Formats a week start date for display in the MonthWeek50px secondary header.
    ...
    private string FormatMonthWeek50pxWeekStartDate(DateTime weekStart)
    {
        try
        {
        ...
        {
            Logger.LogError($"Error formatting MonthWeek50px week start date {weekStart:yyyy-MM-dd}: {ex.Message}");
