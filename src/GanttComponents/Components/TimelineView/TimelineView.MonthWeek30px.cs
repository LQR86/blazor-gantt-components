...
/// <summary>
/// MonthWeek30px pattern implementation for TimelineView.
/// Handles zoom level: MonthWeekOptimal30px (level 28).
/// Primary Header: Month-Year ("February 2025", "March 2025")
/// Secondary Header: Week start dates ("2/17", "2/24", "3/3") - Monday dates
/// Cell Width: 35px week cells (5px day width × 7 days/week)
/// INTEGRAL DAY WIDTHS ONLY - no fractional calculations for better visuals
/// </summary>
public partial class TimelineView
{
    /// <summary>
    /// Main rendering method for MonthWeek30px pattern.
    /// Combines primary (month-year) and secondary (week start dates) headers.
    /// </summary>
    /// <returns>Complete SVG markup for MonthWeek30px headers</returns>
    private string RenderMonthWeek30pxHeaders()
    {
        try
        {
            Logger.LogDebugInfo($"MonthWeek30px pattern rendering - StartDate: {StartDate}, EndDate: {EndDate}, DayWidth: {DayWidth}");

            var primaryHeader = RenderMonthWeek30pxPrimaryHeader();
            var secondaryHeader = RenderMonthWeek30pxSecondaryHeader();

            return $@"
                <!-- MonthWeek30px Pattern Headers -->
                <g class=""monthweek-30px-headers"">
                    {primaryHeader}
                    {secondaryHeader}
                </g>";
        }
        ...
        {
            Logger.LogError($"Error rendering MonthWeek30px headers: {ex.Message}");
            return $"<!-- Error in MonthWeek30px pattern: {ex.Message} -->";
        }
    }

    /// <summary>
    /// Renders the primary header with month-year displays for MonthWeek30px.
    ...
    private string RenderMonthWeek30pxPrimaryHeader()
    {
        var monthPeriods = GenerateMonthWeek30pxMonthPeriods();
        ...
        return $@"
            <!-- Primary Header: Month-Year (MonthWeek30px) -->
            <g class=""monthweek-30px-primary-header"">
                {string.Join("\n                ", headerElements)}
            </g>";
    }

    /// <summary>
    /// Renders the secondary header with week start dates for MonthWeek30px.
    ...
    private string RenderMonthWeek30pxSecondaryHeader()
    {
        var weekPeriods = GenerateMonthWeek30pxWeekPeriods();
        var headerElements = new List<string>();
        ...
        return $@"
            <!-- Secondary Header: Week Start Dates (MonthWeek30px) -->
            <g class=""monthweek-30px-secondary-header"">
                {string.Join("\n                ", headerElements)}
            </g>";
    }

    /// <summary>
    /// Generates month periods for MonthWeek30px timeline range.
    ...
    private List<HeaderPeriod> GenerateMonthWeek30pxMonthPeriods()
    {
        ...
        while (current <= EndDate)
        {
            ...
            if (visibleStart <= visibleEnd)
            {
                var period = new HeaderPeriod
                {
                    ...
                    Label = FormatMonthWeek30pxMonthYear(monthStart)
                };
    ...
    /// <summary>
    /// Generates week periods for MonthWeek30px pattern.
    ...
    private List<HeaderPeriod> GenerateMonthWeek30pxWeekPeriods()
    {
        var periods = new List<HeaderPeriod>();
        ...
        while (current <= EndDate)
        {
            ...
            if (visibleStart <= visibleEnd)
            {
                var period = new HeaderPeriod
                {
                    ...
                    Label = FormatMonthWeek30pxWeekStartDate(weekStart)
                };
    ...
    /// <summary>
    /// Formats a month-year for display in the MonthWeek30px primary header.
    ...
    private string FormatMonthWeek30pxMonthYear(DateTime date)
    {
        try
        {
        ...
        {
            Logger.LogError($"Error formatting MonthWeek30px month-year {date:yyyy-MM-dd}: {ex.Message}");
            return $"{date:MMM yyyy}";
        }
    }

    /// <summary>
    /// Formats a week start date for display in the MonthWeek30px secondary header.
    ...
    private string FormatMonthWeek30pxWeekStartDate(DateTime weekStart)
    {
        try
        {
        ...
        {
            Logger.LogError($"Error formatting MonthWeek30px week start date {weekStart:yyyy-MM-dd}: {ex.Message}");
